...
String firston = "a";  // strings for serial communication to Max
String firstoff = "b";
String secondon = "c";
String secondoff = "d";
...
void fan1check() {
  if (millis() - f1reset >= f1target && f1started == false) { // if time since last reset exceeds target time:
    ...
    Serial.println(firston); // send signal to Max
...
//    Serial.println(sen1);
    if (sen1 > 270) {
      ...
      Serial.println(firstoff); // signal Max
...
void fan2check() {
  if (millis() - f2reset >= f2target && f2started == false) { // if time since last reset exceeds target time:
    ...
    Serial.println(secondon); // signal Max
...
//    Serial.println(sen2);
    if (sen2 > 270) {
      ...
      Serial.println(secondoff); // signal Max
